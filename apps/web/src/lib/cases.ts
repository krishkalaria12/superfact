...
export async function buildDemoCases(): Promise<DemoCase[]> {
  ...
  /**
   * One contradiction the second pass defused. `priorVerdict` being set is the whole signal: a
   * first pass said these conflict and a qualifier turned out to explain both.
   */
  const reconciled = await firstEdge(
    and(eq(edges.verdict, "reconciles"), isNotNull(edges.priorVerdict)),
  );
  ...
  return [
    ...
    },
    {
      ...
      edge: reconciled,
      fact: null,
      note: reconciled ? null : "No contradiction has been downgraded yet.",
    },
    {
      key: "failure",
