...
export const inngest = new Inngest({
  ...
  // Passed rather than left to the SDK's own `process.env` lookup, so that these two names go
  // through the same validated env as every other secret and a missing one fails where the rest
  // of the contract lives instead of inside the serve handler.
